...
class IfThen(BaseEvaluable):
    ...
    def evaluate(self, file: File) -> ValidationResult:
        ...
        if not condition_result.valid:
            return ValidationResult(valid=True, reference="", severity=Severity.SKIPPED,
                                    message="Condition not met, assertion skipped.", target="")
        then_result = self.then.evaluate(file)
        if then_result.valid:
            return ValidationResult(valid=True, reference="", severity=Severity.INFO,
                                    message="Condition met and assertion passed.", target="")
        return ValidationResult(valid=False, reference="", severity=Severity.ERROR,
                                message="Condition met but assertion failed.", target="")
...
class IfThenElse(BaseEvaluable):
    ...
    def evaluate(self, file: File) -> ValidationResult:
        ...
        if condition_result.valid:
            ...
            if then_result.valid:
                return ValidationResult(valid=True, reference="", severity=Severity.INFO,
                                        message="Condition met and 'then' assertion passed.", target="")
            return ValidationResult(valid=False, reference="", severity=Severity.ERROR,
                                    message="Condition met but 'then' assertion failed.", target="")
        else:
            else_result = self.else_.evaluate(file)
            if else_result.valid:
                return ValidationResult(valid=True, reference="", severity=Severity.INFO,
                                        message="Condition not met and 'else' assertion passed.", target="")
            return ValidationResult(valid=False, reference="", severity=Severity.ERROR,
                                    message="Condition not met and 'else' assertion failed.", target="")
...
class OneOf(BaseAssertionGroup):
    ...
    def evaluate(self, file: File) -> ValidationResult:
        ...
        if passing_count == 1:
            return ValidationResult(valid=True, reference="", severity=Severity.INFO,
                                    message="Exactly one assertion passed in 'one_of'.", target="")
        return ValidationResult(valid=False, reference="", severity=Severity.ERROR,
...
class Exactly(BaseAssertionGroup):
    ...
    def evaluate(self, file: File) -> ValidationResult:
        ...
        if passing_count == self.count:
            return ValidationResult(valid=True, reference="", severity=Severity.INFO,
                                    message=f"Exactly {self.count} assertions passed.", target="")
        return ValidationResult(valid=False, reference="", severity=Severity.ERROR,
